	...
	<table width="800" align="center" cellpadding="0" cellspacing="0"
		...
		<c:forEach items="${users.datas }" var="u">
			<tr>
				...
				<td><c:if test="${u.type eq 0 }">普通用户</c:if> <c:if
						test="${u.type eq 1 }">管理员</c:if> <a
					href="user.do?method=changeType&id=${u.id }">变更</a></td>
				<td><a href="user.do?method=updateInput&id=${u.id }">修改</a> <a
					href="user.do?method=delete&id=${u.id }">删除</a></td>
			</tr>
		</c:forEach>
		<tr>
