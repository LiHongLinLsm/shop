...
<body>
	...
	<table width="800" class="thin-border" align="center" cellpadding="0"
		cellspacing="0">
		<tr>
		...
		<tr>
			<td>用户类型: <c:if test="${user.type eq 0 }">普通用户</c:if> <c:if
					test="${user.type eq 1 }">管理员</c:if>
			</td>
		</tr>
		<tr>
			<td><a href="orders.do?method=userList&id=${loginUser.id }">用户订单查询</a>
			</td>
		</tr>
		<tr>
			<td>用户联系地址 <c:if test="${user.id eq loginUser.id }">
			...
			<c:forEach items="${user.addresses }" var="address">
				<tr>
					<td>${address.name }[${address.postcode }](${address.phone })
						...
						&nbsp;<a
						href="address.do?method=delete&id=${address.id}&userId=${user.id}">删除</a>
					</td>
				</tr>
			</c:forEach>
		</c:if>
